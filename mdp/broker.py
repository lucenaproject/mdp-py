...
from mdp import error, protocol
...
class Broker:
    ...
    def __init__(self, bind=DEFAULT_BIND_URL,
    ...
    def run(self):
        """
        Run in a main loop handling all incoming requests,
        ...
        while True:
            ...
            try:
                ...
                if self._socket is None or self._stop:
                    self._log.debug(
                        "Socket has been closed or broker has been shut down, "
                        "breaking from recv loop"
                    )
                    break
                continue
    ...
    def _handle_worker_disconnect(self, worker_id):
        ...
        try:
            ...
            self._log.info(
                "Got DISCONNECT from unknown worker: %d; ignoring",
                id_to_int(worker_id)
            )
    ...
    def _purge_expired_workers(self):
        ...
        """
        for worker in list(self._services.expired_workers()):
            self._log.debug(
                "Worker %d timed out after %0.2f sec, purging",
                id_to_int(worker.id),
                self._busy_worker_timeout if worker.is_busy
...
class Service:
    """
    Service objects manage all workers that can handle a specific service,
    as well as a queue of MDP Client requests to be handled by this service
    """
    def __init__(self):
        self._queue = deque()
...
class ServicesContainer:
    ...
    def __init__(self, busy_workers_timeout=protocol.DEFAULT_BUSY_WORKER_TIMEOUT):
        ...
        self._services = defaultdict(Service)
        self._workers = OrderedDict()
        self._busy_workers = dict()
    ...
    def dequeue_pending(self):
        """
        Pop ready message-worker pairs from all service queues so they
        can be dispatched
        """
    ...
    def set_worker_busy(self, worker_id, expire_at):
        """
        Mark a worker as busy - that is currently processing a request
        and not available for more work
        """
    ...
    def expired_workers(self):
        """
        Get iterator of workers that have expired (no heartbeat received in
        too long) and busy workers that have not returned in too long
        """
        now = time.time()
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        help='Heartbeat interval (sec)',
        type=float,
        default=protocol.DEFAULT_HEARTBEAT_INTERVAL
    )
    parser.add_argument(
        '-t', '--heartbeat-timeout',
        help='Heartbeat timeout (sec)',
        type=float,
        default=protocol.DEFAULT_HEARTBEAT_TIMEOUT
    )
    parser.add_argument(
        '-B', '--busy-worker-timeout',
        help='Timeout for busy workers (sec)',
        type=float,
    ...
    if args.heartbeat_timeout % args.heartbeat_interval != 0.0:
        raise parser.error(
            "--heartbeat-timeout ({}) must be a multiple of "
